...
const NewLink = ({ username }) => {
  ...
  const axiosFunc = () => {
    setAdded(false)
    axios
      .patch(`http://localhost:5000/users/${username}`, values, config)
      .then(res => {
      ...
      <form onSubmit={handleSubmit}>
        <div className='newlink-buttons'>
          <input
            ...
            placeholder={'Title'}
            className={
              errors.linkTitle ? 'user-input error-text' : 'user-input'
            }
          />
          <input
            type='text'
            ...
            onChange={handleChange}
            placeholder={'URL'}
            className={errors.url ? 'user-input error-text' : 'user-input'}
          />
