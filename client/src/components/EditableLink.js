...
const EditableLink = ({ id, username, link, rerender }) => {
  ...
  const axiosFunc = () => {
    axios
      .patch(`http://localhost:5000/users/${username}/editlink`, values, config)
      .then(res => {
        if (editTitle === true) {
          toggleEditTitle()
        }
  ...
  const deleteLink = () => {
    axios
      .patch(
        `http://localhost:5000/users/${username}/deletelink`,
        { _id: id },
        config
      )
      .then(res => {
            ...
            <form onSubmit={handleSubmit}>
              <div>
                <input
                  ...
                  placeholder={'Title'}
                  className={
                    errors.linkTitle ? 'error-url-text' : 'edit-link-input'
                  }
                />
              </div>
            </form>
